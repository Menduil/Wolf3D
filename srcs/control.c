...
void	control_next(t_env *env, const Uint8 *keystates)
{
	...
	if (keystates[SDL_SCANCODE_D])
		p_limits(env, env->p.xpos - round(env->cos_t[alpha_s] *
		(env->p.speed / 2)),
		env->p.ypos + round(env->sin_t[alpha_s] * (env->p.speed / 2)));
	else if (keystates[SDL_SCANCODE_A])
		p_limits(env, env->p.xpos + round(env->cos_t[alpha_s] *
		(env->p.speed / 2)), env->p.ypos -
		round(env->sin_t[alpha_s] * (env->p.speed / 2)));
...
void	control(t_env *env, const Uint8 *keystates)
{
	...
	if (keystates[SDL_SCANCODE_W])
		p_limits(env, env->p.xpos +
		round(env->cos_t[env->p.alpha] * env->p.speed),
